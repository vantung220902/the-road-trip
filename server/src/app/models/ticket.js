...
class Ticket {
        ...
        return new Promise((resolve, reject) => {
            ...
            tickets.number,tickets.place,user.fullName from tickets
             INNER JOIN user on tickets.idAuthor = user.id
              where tickets.sold < tickets.number and CURRENT_DATE <= tickets.dateEnd and tickets.deleted= false and status = true
               LIMIT ?,? `,
    ...
    getNumber() {
        return new Promise((resolve, reject) => {
            ...
            tickets.sold < tickets.number and CURRENT_DATE <= tickets.dateEnd and tickets.deleted= false and status = 1 `,
                (err, res) => {
                    if (err) throw err;
                    return resolve(res);
                })
